...
MODULES: dict[str, dict] = {
    "shmtu_ocr": {
        "target_path": "../shmtu_ocr/src/main/cpp/3rdparty",
        "libs": [
            {
                "name": "ncnn",
                "owner": "Tencent",
...
def process_module(module_name: str, module_cfg: dict, script_dir: Path, args: argparse.Namespace) -> None:
    target_dir = (script_dir / module_cfg["target_path"]).resolve()
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Download and extract Android native library archives for project modules."
    )
    parser.add_argument(
        "--module",
        dest="modules",
        ...
        help=f"Module to process (default: all). Available: {', '.join(MODULES.keys())}.",
    )
    parser.add_argument(
        "--skip-download",
        action="store_true",
    ...
    selected_modules = args.modules if args.modules else list(MODULES.keys())

    for module_name in selected_modules:
        process_module(module_name, MODULES[module_name], script_dir, args)
