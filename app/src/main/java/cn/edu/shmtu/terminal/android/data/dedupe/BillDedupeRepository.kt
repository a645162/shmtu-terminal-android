...
class BillDedupeRepository @Inject constructor(
    ...
    suspend fun dedupeAccount(identityId: Long, accountId: Long): Pair<Int, Int> {
        ...
        val before = identityDb.billDao().getCountByAccountId(accountId)
        val removed = identityDb.billDao().dedupeByTransactionNoForAccount(accountId)
        val after = identityDb.billDao().getCountByAccountId(accountId)
        return Pair(after, removed)
    }
}
